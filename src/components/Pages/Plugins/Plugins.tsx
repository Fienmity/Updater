...
export default () => {
                ...
                onConfirm: () => {
                    availableOptions.forEach(option => {
                        installPlugin(option.updater.plugin, ({ data }) => {
                            ...
                            ?   (() => {
                                setPlugins(previous => previous.filter((item: ManifestType) => item.name!=option.name))
                                Dialog.show({
    ...
    return <>
        ...
        <ExitWrapper component={<View style={{ marginBottom: 100 }}>
            ...
            />
            {plugins.filter(item => item.name.toLowerCase().includes(query)).map(plugin => <PluginItem 
                plugin={{
                    name: plugin.name,
                    version: plugin.version,
                    changelog: plugin.updater?.changelog,
                    download: plugin.updater.plugin,
                    color: plugin?.color
                }} 
                isBulk={isBulk}
                onInstall={(plugin: ManifestType) => setPlugins(previous => previous.filter(item => item.name!=plugin.name))}
            />)}
        </View>} />
